...
SimulMajNum = list[list[dict[str, int]]]  # [j][a][rn]
...
def create_variables(size: int, sigma: Signature, A: Structure) -> Variables:
    ...
    return Variables(pi, pr, hc, is_ex, is_maj, is_num, simul, maj_sim, num_sim, defect, ex_def, maj_def, num_def)
...
def successor_constraints(size: int, A: Structure, sigma: Signature, v: Variables):
    ...
    for a in ind(A):
        for pInd2 in range(size):
            for pInd in range(pInd2):
                pass
                yield [num_def[pInd][pInd2][a], -pi[pInd][pInd2], -is_num[pInd][pInd2],
                       link_num[pInd2][a]]  # HW: paper (9) / (9-NUM)
    for a in ind(A):
        for pInd2 in range(size):
            for rn in rolenames(sigma):
                pass
                yield [-link_num[pInd2][a], -pr[rn][pInd2], num_sim[pInd2][a][rn]]  # HW: paper (9) / (9-NUM)
            ...
            for a in ind(A):
                ...
                for b, rn in A.rn_ext[a]:
                    if rn in rolenames(sigma):
                        ...
                        yield [-is_num[pInd][pInd2], -num_sim[pInd2][a][rn], -num_def[pInd][pInd2][a]]  # 10-NUM
        ...
        for pInd2 in range(pInd + 1, size):
            for a in ind(A):
                for rn in rolenames(sigma):
                    ...
                    yield [-is_num[pInd][pInd2], -defect[pInd][pInd2][a], -pr[rn][pInd2],-num_sim[pInd2][a][rn]]  # 12-MAJ
